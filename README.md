...
If you want to change this behavior, as for big infrastructures this will create a lot of variables, you can use the `--module-varibles path/to/file` and the file will have the list of attributes that you want to actually be used as variables:
...
**Trick:** If you want to generate the HCL but not as a module but still have different files, you can use the `--module` with `--module-variables` and have an nonexistent attribute so it'll not replace anything for variables.
